...
# Inner ellipsoid (smaller)
inner_scale = 0.8  # Adjust the scale as needed
x_inner_ellipsoid = inner_scale * a * np.cos(u) * np.sin(v) + x0
y_inner_ellipsoid = inner_scale * b * np.sin(u) * np.sin(v) + y0
z_inner_ellipsoid = inner_scale * c * np.cos(v) + z0

# Outer ellipsoid (larger)
outer_scale = 1.2  # Adjust the scale as needed
x_outer_ellipsoid = outer_scale * a * np.cos(u) * np.sin(v) + x0
y_outer_ellipsoid = outer_scale * b * np.sin(u) * np.sin(v) + y0
z_outer_ellipsoid = outer_scale * c * np.cos(v) + z0

# Plot ellipsoids
ax.plot_wireframe(x_ellipsoid, y_ellipsoid, z_ellipsoid, color='green', alpha=0.5, label='Fitted Ellipsoid')
ax.plot_wireframe(x_inner_ellipsoid, y_inner_ellipsoid, z_inner_ellipsoid, color='red', alpha=0.5, label='Inner Ellipsoid')
ax.plot_wireframe(x_outer_ellipsoid, y_outer_ellipsoid, z_outer_ellipsoid, color='purple', alpha=0.5, label='Outer Ellipsoid')
